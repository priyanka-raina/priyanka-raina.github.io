...
Modern AI applications, that typically use neural networks (NNs), have a very diverse set layers and connections. Many networks, such as HDRNet for image enhancement or PWCNet for optical flow, combine traditional image and video processing operators with convolutional and fully-connected layers to achieve state of the art accuracies. Existing NN hardware accelerators, with arrays of multiply-accumulate units, have very limited configurability and are optimized to only run the convolutional and fully-connected layers, and therefore cannot support this growing space of hybrid applications. Configurable accelerators, such as coarse-grained reconfigurable arrays (CGRAs), on the other hand offer an excellent alternative for accelerating these applications. However, coming up with an optimal CGRA design while exploring the trade-off between configurability and performance requires a large amount of manual effort. This work creates a framework for automatic design space exploration of processing elements (PEs) in CGRAs. To find interesting PEs, we feed a dataflow graph representation of the application into a frequent subgraph mining algorithm. The frequency of the resulting subgraphs represents how often that sequence of operations occurs in the application. We then use subgraph merging techniques to allow for the acceleration of multiple subgraphs from the same application or different applications. By tuning which subgraphs are merged, we can explore the design space between more specialized and more general CGRA PEs. Once we have candidate PEs, we generate PEak programs for them. The PEak compiler  then automatically generates the PE hardware (Verilog) and a set of rewrite rules that map operators in the dataflow IR to the hardware PEs. So as the application analysis engine generates new PEs, we automatically generate PE hardware and compiler and obtain power, performance and area results for an application running on a CGRA with these new PEs. 
...
*Symposium on VLSI Circuits (VLSIC)*, June 2020. [Talk](https://stanford.box.com/s/n3pcrulsookozhpnxqwpkkbjhlaea1m3), [Slides](https://stanford.box.com/s/igmr2okv8ewdc4u4z1j92asb9fpozx56), [Demo](https://stanford.box.com/s/jgwhx5vszvxgijkh0xyen7q8dfmx7jue)
